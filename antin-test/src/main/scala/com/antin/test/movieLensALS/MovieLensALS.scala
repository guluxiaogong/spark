...
object MovieLensALS {
  def main(args: Array[String]) {
    ...
    val myRatings = loadRatings("D:\\ZHS\\data\\mllib_001\\data\\class8\\movielens\\personalRatings.txt")
    val myRatingsRDD = sc.parallelize(myRatings, 1)

    // 样本数据目录
    //val movieLensHomeDir = args(0)
    val movieLensHomeDir = "D:\\ZHS\\data\\mllib_001\\data\\class8\\movielens\\data"

    // 装载样本评分数据，其中最后一列Timestamp取除10的余数作为key，Rating为值,即(Int,Rating)//1::1193::5::978300760
    val ratings = sc.textFile(new File(movieLensHomeDir, "ratings.dat").toString).map { line =>
      val fields = line.split("::")
      (fields(3).toLong % 10, Rating(fields(0).toInt, fields(1).toInt, fields(2).toDouble))
    }

    // 装载电影目录对照表（电影ID->电影标题）//1::Toy Story (1995)::Animation|Children's|Comedy
